...
- Name: Text on screen when there is no mail in the in box
- Location: C:\Users\Pteta\Hexaware\SnailMail\snail-mail-fe\playwright_tests\inbox.spec.tsx:37:1
...
Call log:
  ...
  - waiting for getByText('No Mail! You\'re all caught up!')
    ...
    at C:\Users\Pteta\Hexaware\SnailMail\snail-mail-fe\playwright_tests\inbox.spec.tsx:39:68
```
...
```ts
  ...
  37 | test("Text on screen when there is no mail in the in box", async ({ page }) => {
  38 |     await page.goto('/') //Go to inbox.tsx
> 39 |     await expect(page.getByText("No Mail! You're all caught up!")).toBeVisible()
     |                                                                    ^ Error: Timed out 5000ms waiting for expect(locator).toBeVisible()
  40 | })
  41 |
  42 | //test 5: Compose component opens when button is clicked
  43 | test("Compose component opens when button is clicked", async ({ page }) => {
  44 |     await page.goto('/') //Go to main page
  45 |     await page.getByRole('button', {name: "Compose Email"}).click() //Open Compose.tsx
  46 |     await expect(page.getByTestId("compose-component")).toBeVisible()
  47 | })
```